...
<body>
    <div id="app">
        ...
        <div class="connection-info">
            <div>Status: <span idz="connectionStatus">Checking...</span></div>
...
bool scanRequested = false;  // Add this global variable at the top of your code

void performWiFiScan(void * parameter) {
  for(;;) {
    // Only scan if either:
    // 1. WiFi is not connected, or
    // 2. A scan was explicitly requested via the button
    if (WiFi.status() == WL_CONNECTED && !scanRequested) {
    ...
    scanComplete = true;
    scanRequested = false;  // Reset the scan request flag
    WiFi.scanDelete();
  }
}

// Modify the handleScanNetworks function as well:
void handleScanNetworks(AsyncWebServerRequest *request) {
  if (scanComplete) {
    request->send(200, "application/json", scanResults);
    scanComplete = false; // Reset for next scan
    scanRequested = true; // Set the flag to request a new scan
  } else {
...
void attemptWiFiConnections() {
    ...
    if (savedSsid.length() > 0) {
        ...
        if (!connectToWiFi(savedSsid, savedPassword)) {
            Serial.println("Failed to connect using stored credentials");
        }
    } else {
